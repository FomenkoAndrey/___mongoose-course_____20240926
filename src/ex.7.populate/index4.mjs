...
async function run() {
  try {
    ...
    try {
      const streets = await Street.create([{ name: 'Main St' }, { name: 'Cathedral St' }])
      const cities = await City.create([{ name: 'New York' }, { name: 'Montreal' }])
      const countries = await Country.create([{ name: 'USA' }, { name: 'Canada' }])
